...
export const productsController =  {
    ...
    addProduct: async (req: Request, res: Response) =>{
        try {
            ...
            if (name && onStock){
                const addedProduct = await prisma.product.create({data: {
                    name: name,
                    onStock: onStock
                }})
    ...
    deleteUniqueProduct: async (req: Request, res: Response)=>{
        try{
            ...
            if (!idToBeDeleted) return res.status(400).json({message: "Id is missing"})
            const itemDeleted = await prisma.product.delete({where: {id: idToBeDeleted}})
